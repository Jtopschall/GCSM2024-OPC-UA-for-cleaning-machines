...
start_times = []
end_times = []
# Hier einstellen, ob gerade Morgens oder Abendsschicht (5:45 oder 13:45)
...
DA_start = pd.Timestamp.now(tz='Europe/Brussels').replace(hour=0, minute=0, second=0, microsecond=0,
                                                          nanosecond=0)  # timedelta -7 entfernt  # -6 for testing, no 15m interval on christmas
...
# summarized method
async def efdm_from_el_price(shift_start_hour, shift_start_minute):
    """
    Analyze electricity prices for the next 8 hours starting from the specified shift start hour.
    Start: Überprüfen der Zeiteinstellungen und welche Tankheizungen angemacht werden sollen
    und wie die Wetterbedingungen (Preise sind) - danach schauen wie man die Preisschranken setzt
    """
    # Sicherheitsfaktor # 0.1 nachts und 0.5 tagsüber ? - ist kein Sicherhietsfaktor sondern ein prozentualer Abschlag
    fsp_1 = 0.25
    fsp_2 = 0.9 # möglicherweise nicht notwendig, weil wir eh nur ein Heizstabpaar während des Betriebs anmachen können
    ...
    adjusted_average_1 = average_price * (1 - fsp_1)
    adjusted_average_2 = average_price * (1 - fsp_2)

    print(f"Durchschnittspreis über 8 Stunden: {average_price:.2f} EUR")
    print(f"Angepasster Durchschnittspreis (1 - fsp): {adjusted_average_1:.2f} EUR")
    print(f"Angepasster Durchschnittspreis (1 - fsp): {adjusted_average_2:.2f} EUR")

    # Durchlauf der 32 Intervalle ab Schichtstartzeitpunkt
    for i in range(num_intervals):
        current_price = price_list[start_index + i]

        # Berechne Start- und Endzeitpunkt für das IntervallflexLoad_index
        ...
        print(f"Aktueller Preis: {current_price:.2f} EUR, "
              f"Vergleichspreis: {adjusted_average_1:.2f} EUR")
        print(f"Aktueller Preis: {current_price:.2f} EUR, "
              f"Vergleichspreis: {adjusted_average_2:.2f} EUR")

        # reward kalkulieren:
        reward = max(average_price * 0.006 - current_price * 0.006, 0)

        if Temp_ST >= 66.0: # 65 grad weil kontrolliert ja nur alle 15 Minuten und nicht das er dann überhitzt wenn er bei 69 grad aufheizt
            print("Spültanktemperatur >= 66°C, Heizstab_ST_12 bleibt unabhängig vom Preis deaktiviert")

        elif current_price <= adjusted_average_1:
        ...
        elif Temp_ST < 50.0: # on time auf 2-3 Minuten hier begrenzen? - nehme Wert aus Referenzprozess (durchschnittliche On-Time der Stäbe wenn unter 50 Grad) - 6 Minuten - auf 7 Minzten angepasst (weil beim ersten versuch sehr knapp)
        ...
        """
        Dürfen und können nur ein Heizstabpaar während des Betriebs ansteuern!
...
async def stop_HS_temp():
    # Anpassen an vorheizen Tagsüber oder Nachts (51 oder 71 °C)
...
async def check_temp_bef_start():
    # asyncio sleep auf 2:45 minuten anpassen (9900)
    await asyncio.sleep(9900)
  # final temperature check 15 minutes before the shift starts - anpassen der Zeit
    Temp_WT, Temp_ST = await read_tank_temperatures()
...
async def efdm_from_Aufheizen_Tagsueber_EIS():
    """
    Anpassungen bei verschiedenen Schichtbeginnen vornehmen
    Find the cheapest 1-hour interval in the 3 hours before the shift for heating purposes.
...
#######################################################################################################################
# Bivalenter Betrieb - Aufheizphase Nachts über Wärmenetz (BHKW) - danach umschalten auf normalen Betrieb #############
#######################################################################################################################

"""async def efdm_from_Nachts_Aufheizen_BB_WN(flexLoad_index):
    
    # Starten mit der Aufheizphase über das rote Netz in der teuersten Strompreisphase
    ...
    # Indizes für den gesamten Zeitraum von 3:00 bis 6:00 Uhr (3 Stunden à 4 Intervalle) - manuell für den richtigen Zeitpunkt einstellen
    start_index = 12 # 3:00 Uhr entspricht dem 13. Intervall (Index 12)
    end_index = 24 # 6:00 Uhr entspricht dem 25. Intervall (Index 24, nicht inklusvie)

    # Sicherstellen, dass genügend Daten vorhanden sind
    if len(price_list) < end_index:
        print("Nicht genügend Daten für den Zeitraum von 3:00 bis 6:00 Uhr vorhanden")

    # Initialize variables to track the maximum cost intervall
    max_cost = float('inf')
    best_interval_start_index = None

    # Schleife durch die Indizes, um das teuerste 1-Stunden-Intervall zu finden
    for i in range(start_index, end_index -3): # -3, da wir vier aufeinanderfolgende Intervalle benötigen
        current_cost = sum(price_list[i:i + 4])
    ...
    # Wenn ein geeignetes Intervall gefunden wurde, berechnen wir Start- und Endzeiten
    if best_interval_start_index is not None:
        best_on_time = pd.Timestamp.now(tz='Europe/Brussels').replace(hour=3, minute=0, second=0, microsecond=0) + timedelta(minutes=(best_interval_start_index - start_index) * 15)
        best_off_time = best_on_time + timedelta(hours=1)

        # Kosten des Intervalls direkt vor Schichtbeginn (5:00 bis 6:00)
        last_hour_cost = sum(price_list[end_index - 4:end_index])

        # reward - Pauschale Kosten durch BHKW im Vorhinein berechnen und mit Strompreisen vergleichen
        reward = max_cost * 0.006 - 0.3

        # Ausgabe der Ergebnisse
        print(f"Startzeitpunkt der Schicht: {pd.Timestamp.now(tz='Europe/Brussels').replace(hour=6, minute=0, second=0, microsecond=0).strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Kosten für das Intervall direkt vor Schichtbeginn: {last_hour_cost:.2f} EUR")
        print(f"Teuerstes Intervall gefunden: {best_on_time.strftime('%Y-%m-%d %H:%M:%S')} - {best_off_time.strftime('%H:%M:%S')}")
        print(f"Gesamtkosten des teuersten Strompreis-Intervalls: {max_cost:.2f} EUR")
        print(f"Berechneter Reward: {reward.4f} EUR")"""
...
async def main():
    ...
    # Was alles eingestellt werden muss vor Umsetzung der Maßnahmen in den Versuchen:
    # In main Schleife nur das aktiv setzen, was gerade ansteht
    # final_check_time auch die richtige Uhrzeit setzen
    # vorher testen welches das günstigste Intervall ist
    # starte 'check_temp_bef_start' als separate asynchrone Aufgabe' auskommentieren wenn:
    # Günstigstes Zeintervall direkt vor Schichtbeginn
    # Wenn Aufheizphase vorbei und Durchlaufbetrieb beginnt
    #Zeiltemperatur bei stop funktion am Tag auf 70 Grad setzen
    # asyncio sleeps richtig anpassen - vor allem bei check_temp_bef_start.. (auf 9900)
