...
tf_price_dict = {'tf1_price': 6012, 'tf2_price': 6021, 'tf3_price': 6027, 'tf4_price': 6033, 'tf5_price': 6039, 'tf6_price': 6045, 'tf7_price': 6051, 'tf8_price': 6057, 'tf9_price': 6063, 'tf10_price': 6069, 'tf11_price': 6075, 'tf12_price': 6081, 'tf13_price': 6087, 'tf14_price': 6093, 'tf15_price': 6099, 'tf16_price': 6105, 'tf17_price': 6111, 'tf18_price': 6117, 'tf19_price': 6123, 'tf20_price': 6129, 'tf21_price': 6135, 'tf22_price': 6141, 'tf23_price': 6147, 'tf24_price': 6153}


#paths to all nodesets
DI_path = 'C:/Users/Max/Desktop/Masterarbeit/programmieren/EnFlexServerDLRA/Opc.Ua.Di.NodeSet2.xml'
IA_path = 'C:/Users/Max/Desktop/Masterarbeit/programmieren/EnFlexServerDLRA/Opc.Ua.IA.NodeSet2.xml'
Machinery_path = 'C:/Users/Max/Desktop/Masterarbeit/programmieren/EnFlexServerDLRA/Opc.Ua.Machinery.NodeSet2.xml'
MT_path = 'C:/Users/Max/Desktop/Masterarbeit/programmieren/EnFlexServerDLRA/Opc.Ua.MachineTool.NodeSet2.xml'
CO2_path = 'C:/Users/Max/Desktop/Masterarbeit/programmieren/EnFlexServerDLRA/adp_mt_co2.xml'
EnFlex_path = 'C:/Users/Max/Desktop/Masterarbeit/programmieren/EnFlexServerDLRA/opc_ua_for_enflex.xml'
DLRAServer_path = 'C:/Users/Max/Desktop/Masterarbeit/programmieren/EnFlexServerDLRA/dlra_server.xml'
...
MappingtablePath = 'C:/Users/Max/Desktop/Masterarbeit/programmieren/EnFlexServerDLRA/EnFlexMappingtable.xlsx'
...
FlexLoads_Objects_dict = {"Heizstab_ST_12": 5022, "Heizstab_ST_34": 5023, "Heizstab_ST_5": 5024, "Heizstab_WT_12": 5004, "Heizstab_WT_34": 5020, "Heizstab_WT_5": 5021, "IR_Strahler_14": 5032, "IR_Strahler_23": 5033}
...
AllLoads_Objects_dict = {"Abluefter": 5039, "Bandantrieb": 5028, "Heizstab_ST_12": 5025, "Heizstab_ST_34": 5026, "Heizstab_ST_5": 5031, "Heizstab_WT_12": 5011, "Heizstab_WT_34": 5029, "Heizstab_WT_5": 5030, "IR_Strahler_14": 5034, "IR_Strahler_23": 5035, "Induktor": 5042, "Kuehlpumpe": 5027, "Luefter_SchwaKo": 5038, "Spritzpumpe_SZ": 5037, "Spritzpumpe_WZ": 5036, "Umlufttrockner": 5040, "Vakuumtrockner": 5041}
...
All_BeginOfCalcualtion_Nodes_dict = {"CO2PerWorkpiece_BeginOfCalculation": 6042, "ElCostPerWorkpiece_BeginOfCalculation": 6035, "RunTimeData_BeginOfCalculation": 6011, "TotalCO2_BeginOfCalculation": 6014, "ElPowerConsumtion_BeginOfCalculation": 6018, "AirConsumption_BeginOfCalculation": 6027}
...
async def update_PLC_values():
    ...
    while tries < 6:
        try:
            await Siemens_DLRA_Server_client.connect()
    ...
    for index, row in Mappingtable.iterrows():
        locals()['N_'+ row[0]] = server.get_node(ua.NodeId(row[1], 8))
        
        if str(row[3]) != "Null":
            locals()['Siemens_N_'+ row[0]] = Siemens_DLRA_Server_client.get_node(str(row[3]))
            locals()['Siemens_V_'+ row[0]] = await locals()['Siemens_N_'+ row[0]].read_value()
        
            try:
                if row[5] == "Bool" and row[2] != "Null":
                    ANode = server.get_node(ua.NodeId(int(row[2]), 8))
                    await ANode.set_value(locals()['Siemens_V_'+ row[0]])
                    await locals()['N_'+ row[0]].set_value(locals()['Siemens_V_'+ row[0]])
                    
                elif row[5] == "Bool":
                    await locals()['N_'+ row[0]].set_value(locals()['Siemens_V_'+ row[0]])
                    
                elif row[5] == "Float" and row[2] != "Null":
                    ANode = server.get_node(ua.NodeId(int(row[2]), 8))
                    await ANode.set_value(float(locals()['Siemens_V_'+ row[0]]))
        ...
        #use simulated values when not available from siemens server
            ...
            try:
                if row[5] == "Bool" and row[2] != "Null":
                    ...
                    await locals()['N_'+ row[0]].set_value(ua.uatypes.Int32(row[4]))
...
async def workpiece_counter():    
    ...
    while True:
        ...
        V_EURpWP_CurrentValue = (await globals()['N_total_CurrentElectricPowerConsumption'].read_value()) * 12 * (1/3600) * globals()['V_El_Price'] * (1/1000000) #eur/MWh, divide for Wh equasion, get from central server
...
async def set_loadchanges(browsename, nodeID, LoadChangeNumber, AssEnflexMeasureID, AssFlexLoadID, power, timestamp, unit):
    ...
    await globals()['N_'+browsename+'_LoadChangesFolder'].add_object(ua.NodeId(nodeID+(20*LoadChangeNumber), 8), ua.QualifiedName(browsename+'_LoadChange'+str(LoadChangeNumber), 8), ua.NodeId(1008, 7), True) #This works
        
    New_LoadChange = server.get_node(ua.NodeId(nodeID+(20*LoadChangeNumber), 8)) 
        
    globals()['N_'+browsename+'_LoadChange'+str(LoadChangeNumber)+'_AssEnFlexMeasureID'] = await New_LoadChange.get_child(ua.QualifiedName("AssociatedEnFlexMeasureID", 7))
    await globals()['N_'+browsename+'_LoadChange'+str(LoadChangeNumber)+'_AssEnFlexMeasureID'].set_value(AssEnflexMeasureID)
        
    globals()['N_'+browsename+'_LoadChange'+str(LoadChangeNumber)+'_AssFlexLoadID'] = await New_LoadChange.get_child(ua.QualifiedName("AssociatedFlexibleLoadID", 7))
    await globals()['N_'+browsename+'_LoadChange'+str(LoadChangeNumber)+'_AssFlexLoadID'].set_value(AssFlexLoadID)
        
    globals()['N_'+browsename+'_LoadChange'+str(LoadChangeNumber)+'_Power'] = await New_LoadChange.get_child(ua.QualifiedName("Power", 7))
    await globals()['N_'+browsename+'_LoadChange'+str(LoadChangeNumber)+'_Power'].set_value(float(power))
        
    globals()['N_'+browsename+'_LoadChange'+str(LoadChangeNumber)+'_Timestamp'] = await New_LoadChange.get_child(ua.QualifiedName("Timestamp", 7))
    await globals()['N_'+browsename+'_LoadChange'+str(LoadChangeNumber)+'_Timestamp'].set_value(datetime.strptime(timestamp, date_format))
        
    globals()['N_'+browsename+'_LoadChange'+str(LoadChangeNumber)+'_Unit'] = await New_LoadChange.get_child(ua.QualifiedName("Unit", 7))
    await globals()['N_'+browsename+'_LoadChange'+str(LoadChangeNumber)+'_Unit'].set_value(unit)
      
        
    #do the same for AllLoadChanges folder:
    await globals()['N_AllLoadChangesFolder'].add_object(ua.NodeId(nodeID+300+(20*LoadChangeNumber), 8), ua.QualifiedName(browsename+'_LoadChange'+str(LoadChangeNumber), 8), ua.NodeId(1008, 7), True) #This works
...
async def create_PlannedMeasure_node(efdm_path, browsename, nodeID):
    ...
    await globals()['N_PlannedEnFlexMeasuresFolder'].add_object(ua.NodeId(nodeID, 8), ua.QualifiedName(browsename, 8), ua.NodeId(1004, 7), True) #This works
    
    New_Measure = server.get_node(ua.NodeId(nodeID, 8))
    
    #write AssociatedFlexLoadID
    globals()['N_'+browsename+'_AssFlexLoadID'] = await New_Measure.get_child(ua.QualifiedName("AssociatedFlexLoadID", 7))
    await globals()['N_'+browsename+'_AssFlexLoadID'].set_value(NewMeasure_FlexLoadID)
    
    #write EnFlexMeasureID
    globals()['N_'+browsename+'_EnFlexMeasureID'] = await New_Measure.get_child(ua.QualifiedName("EnFlexMeasureID", 7))
    ...
    globals()['N_'+browsename+'_LoadChangesFolder'] = await New_Measure.get_child(ua.QualifiedName("LoadChanges", 7))
    
    #get first LoadChange node obj
    globals()['N_'+browsename+'_LoadChange1'] = await globals()['N_'+browsename+'_LoadChangesFolder'].get_child(ua.QualifiedName("<LoadChange1>", 7))
    ...
    while i < len(NewMeasure_AllLoadChanges):
        ...
        await set_loadchanges(browsename, nodeID, NewLoadChange_Number, NewMeasure_EnFlexMeasureID, NewMeasure_FlexLoadID, power, timestamp, unit)
        
        #schedule to move measure to completed once last load change happened
        if i == (len(NewMeasure_AllLoadChanges) - 1):
            scheduler.enterabs(datetime.strptime(timestamp, date_format), 1, move_completed_measure, argument=(nodeID, browsename))
...
async def move_completed_measure(NodeID_of_measure_to_move, browsename_of_measure_to_move):
    ...
    while i < len(LoadChanges):
        locals()['V_LoadChange' + str(i+1) + '_Power_to_move'] = await LoadChanges[i].get_child(ua.QualifiedName("Power", 7))
        locals()['V_LoadChange' + str(i+1) + '_Timestamp_to_move'] = await LoadChanges[i].get_child(ua.QualifiedName("Timestamp", 7))
        locals()['V_LoadChange' + str(i+1) + '_Unit_to_move'] = await LoadChanges[i].get_child(ua.QualifiedName("Unit", 7))
        i += 1
    ...
    await globals()['N_CompletedEnFlexMeasuresFolder'].add_object(ua.NodeId(NodeID_of_measure_to_move, 8), ua.QualifiedName(browsename_of_measure_to_move, 8), ua.NodeId(1004, 7), True) #This works
    
    New_Completed_Measure = server.get_node(ua.NodeId(NodeID_of_measure_to_move, 8))
    
    #write AssociatedFlexLoadID
    N_New_Completed_Measure_AssociatedFlexLoadID  = await New_Completed_Measure.get_child(ua.QualifiedName("AssociatedFlexLoadID", 7))
    await N_New_Completed_Measure_AssociatedFlexLoadID.set_value(V_FlexloadID_to_move)
    
    #write EnFlexMeasureID
    N_New_Completed_Measure_EnFlexMeasureID = await New_Completed_Measure.get_child(ua.QualifiedName("EnFlexMeasureID", 7))
    ...
    #get first LoadChange node obj
    New_Completed_Measure_LoadChangesFolder_LoadChange1 = await New_Completed_Measure_LoadChangesFolder.get_child(ua.QualifiedName("<LoadChange1>", 7))
    ...
    i=0
    while i < len(LoadChanges):
        await New_Completed_Measure_LoadChangesFolder.add_object(ua.NodeId(13000+20*i, 8), ua.QualifiedName("LoadChange"+str(i+1), 8), ua.NodeId(1008, 7), True) #This works
...
def on_efdm_created(event):
    ...
    path = event.src_path
    Split = path.split(".")
    MeasureNumber = Split[0][-1]
    
    asyncio.run(create_PlannedMeasure_node(event.src_path, "Measure"+MeasureNumber, 9999+(int(MeasureNumber)-1)*700))
...
def create_efdm_watchdog():       
    ...
    efdmObserver.schedule(The_event_handler, "C:/Users/Max/Desktop/Masterarbeit/programmieren/MeasuresExchangeFolder", True)
    efdmObserver.start()
    ...
    for k, v in flexloadID_dict:
        if v == FlexLoad_to_automate:
            ...
            if Power_to_automate == 0:
                ...
                try: 
                    N_Power_Value_to_automate = await globals()['N_FlexLoads_'+k].get_child(ua.QualifiedName("Power", 7))
                    await N_Power_Value_to_automate.set_value(Power_to_automate)
                except:
                    pass
            print("LoadChange for " + FlexLoad_to_automate + " completed")
        else:
            pass
...
async def flexload_automator():
    ...
    if len(children) == globals()['Initial_Children']:
        ...
        while i < len(children):
            ...
            scheduler.enterabs(V_Time_to_automate, priority_number, change_flexload, argument=(V_FlexLoad_to_automate, V_Power_to_automate)) 
            priority_number += 1 #count up priority to schedule on/off loads correctly
            
            print("successfully scheduled load change for " + V_FlexLoad_to_automate + ": " + str(V_Time_to_automate) + ", " + str(V_Power_to_automate) + "W")
            i += 1
...
async def main():
    print('')
    print('###############################################################################################################')
    print('This is the BvL DLRA server, which executes energy flexibility measures and provides data for energy management')
    print('###############################################################################################################')
    ...
    while True: 
        ...
        print(str(len(scheduler.queue))+" entries in scheduler queue:")
